...
class Graph():
    ...
    def printMST(self, parent, d_temp, t):
        ...
        return (sum_weight  + min1 + min2) %1000
    ...
    def primMST(self, d_temp, t):
        ...
        for c in range(self.v):
            ...
            for v in range(self.v):
                ...
                # Update the key only if v graph[u][v] is smaller than key[v]
...
# Determine weight of that mat and connect it with the visited node and 0th node
# Print a Algorithm used for MST (Greedy approach)
    ...
    if num != 0:
        ...
        for i in range(V):
            if i not in visited:
                d_temp[i] = key
                key = key + 1
        # Build the graph for unvisited nodes
        i = 0
        for i in range(V):
            for j in range(V):
...
def startTSP(graph, tree, V):
    ...
    tree.create_node("1", "1", data=TreeNode(0, 1, heu, heu, -1)) # Create first node in the tree i.e. 0th city
    fringe_list = {} # Fringe List(Dictionary)(FL)
    fringe_list[key] = FringMode(0, heu) # Adding first node in FL
    key = key + 1
    ...
    while goalState == 0:
        ...
        if toExpand.c_no == 0 and path == 1:
            ...
            while j < V:
                if j != toExpand.c_no:
                    h_val = heuristic(tree, toExpand.c_id, j, V, graph) # Heuristic calc
                    f_val = val + graph[j][toExpand.c_no] + h_val # g(parent) + g(parent->child) + h(child)
                    fringe_list[key] = FringMode(j, f_val)
                    tree.create_node(str(j), str(key), parent=str(toExpand.c_id), data=TreeNode(j, key, f_val, h_val, toExpand.c_id))
                    key = key + 1
                j = j + 1
